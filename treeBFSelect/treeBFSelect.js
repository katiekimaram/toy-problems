...
Tree.prototype.BFSelect = function(filter) {
  ...
  var traverse = (branch, depth) => {
      if (filter(branch.value, depth)) {
        arr.push(branch.value);
      }

    // if child branch exist, increase depth
    if (branch.children) { depth++; }

    branch.children.forEach(childBranch => traverse(childBranch, depth));
  };
  ...
  // return [filter(this.value, 0) ? this.value, ''].concat(this.children.filter((branch) => {filter(branch.value, 0)} ));
};
