...
Range.prototype.size = function () {
  return Math.round(this.end ? this.end / this.step : this.start / this.step);
};

Range.prototype.each = function (callback) {
  var i = this.start;

  this.step = this.start > this.end && this.step > 0 ? this.step * -1 : this.step;

  while ((i <= this.end && this.step > 0) || (i >= this.end && this.step < 0)) {
    callback(i);
    i += this.step;
  }
};
...
Range.prototype.includes = function (val) {
  return isNaN(this.end) ? this.start === val : val < this.end ? val % this.step === 0 : false;
};
